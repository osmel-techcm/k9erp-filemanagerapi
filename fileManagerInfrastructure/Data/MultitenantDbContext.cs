...
namespace fileManagerInfrastructure.Data
{
    public class MultitenantDbContext : DbContext
    {
        ...
        private string getConnectionString()
        {
            ...
            connectionString = connectionString.Replace(@"{{SERVER}}", server).Replace(@"{{DATABASE}}", idTenant + "-fm").Replace(@"{{USER}}", user).Replace(@"{{PASSWORD}}", password);
